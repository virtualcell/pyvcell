...
def test_vcml_reader(vcml_spatial_model_1d_path: Path) -> None:
    ...
    with open(vcml_spatial_model_1d_path, "r") as f:
        xml_string = f.read()
    ...
    assert ({p.name: p.value for p in r1.kinetics.kinetics_parameters} == {'J': '((Kf * s0) - (Kr * s2))', 'I': 0,
                                                                           'netValence': 1, 'Kf': 1, 'Kr': 1})
    ...
    assert ({p.name: p.value for p in r2.kinetics.kinetics_parameters} == {'J': '((Kf * s2) - (Kr * s3))', 'I': 0,
                                                                           'netValence': 1, 'Kf': 1, 'Kr': 1})
