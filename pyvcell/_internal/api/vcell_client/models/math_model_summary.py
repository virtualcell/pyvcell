    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError(
                    "Error due to additional fields (not defined in MathModelSummary) in the input: " + _key
                )

        _obj = cls.model_validate({
            "version": Version.from_dict(obj["version"]) if obj.get("version") is not None else None,
            "keyValue": obj.get("keyValue"),
            "modelInfo": MathModelChildSummary.from_dict(obj["modelInfo"])
            if obj.get("modelInfo") is not None
            else None,
            "softwareVersion": VCellSoftwareVersion.from_dict(obj["softwareVersion"])
            if obj.get("softwareVersion") is not None
            else None,
            "publicationInfos": [PublicationInfo.from_dict(_item) for _item in obj["publicationInfos"]]
            if obj.get("publicationInfos") is not None
            else None,
