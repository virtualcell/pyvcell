    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError(
                    "Error due to additional fields (not defined in StandardExportInfo) in the input: " + _key
                )

        _obj = cls.model_validate({
            "outputContext": [AnnotatedFunctionDTO.from_dict(_item) for _item in obj["outputContext"]]
            if obj.get("outputContext") is not None
            else None,
            ...
            "geometrySpecs": GeometrySpecDTO.from_dict(obj["geometrySpecs"])
            if obj.get("geometrySpecs") is not None
            else None,
            "timeSpecs": TimeSpecs.from_dict(obj["timeSpecs"]) if obj.get("timeSpecs") is not None else None,
            "variableSpecs": VariableSpecs.from_dict(obj["variableSpecs"])
            if obj.get("variableSpecs") is not None
            else None,
        })
        return _obj
