    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError(
                    "Error due to additional fields (not defined in N5ExportRequest) in the input: " + _key
                )

        _obj = cls.model_validate({
            "standardExportInformation": StandardExportInfo.from_dict(obj["standardExportInformation"])
            if obj.get("standardExportInformation") is not None
            else None,
