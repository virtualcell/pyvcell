    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.external_data_id_sim_refs:
            for _item_external_data_id_sim_refs in self.external_data_id_sim_refs:
                if _item_external_data_id_sim_refs:
                    _items.append(_item_external_data_id_sim_refs.to_dict())
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError(
                    "Error due to additional fields (not defined in FieldDataReference) in the input: " + _key
                )

        _obj = cls.model_validate({
            "externalDataIdentifier": ExternalDataIdentifier.from_dict(obj["externalDataIdentifier"])
            if obj.get("externalDataIdentifier") is not None
            else None,
            "externalDataAnnotation": obj.get("externalDataAnnotation"),
            "externalDataIDSimRefs": [KeyValue.from_dict(_item) for _item in obj["externalDataIDSimRefs"]]
            if obj.get("externalDataIDSimRefs") is not None
            else None,
        })
        return _obj
