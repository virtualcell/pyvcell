...
def get_mask(pde_dataset: PdeDataSet, mesh: CartesianMesh):
...
def get_raw_metadata(zarr_dataset: zarr.Group | zarr.Array) -> dict:
    return zarr_dataset.attrs.asdict()["metadata"]
...
def slice_dataset(
    ...
    return slice_dataset_2d(channel, dataset, time_index, z_index) if z_index is not None \
...
def slice_dataset_2d(
    ...
    return dataset[time_index, channel.index, z_index, :, :]


def slice_dataset_3d(
        channel: ChannelMetadata,
        dataset: zarr.Group | zarr.Array,
        time_index: int
) -> NDArray3D:
    return dataset[time_index, channel.index, :, :, :]
